...
$app->post('/api/Bitly/updateUser', function ($request, $response) {
    ...
    if(!empty($post_data['args']['login'])){
        $data['login'] = $post_data['args']['login'];
    }
    if(!empty($post_data['args']['fullName'])){
        $data['full_name'] = $post_data['args']['fullName'];
    }

    $query_str = $settings['api_url'] . "user/info";
